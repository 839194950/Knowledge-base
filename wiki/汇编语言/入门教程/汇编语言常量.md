...
tags: [汇编, 常量, x86, 宏]
sources: [raw/articles/汇编语言常量.md]
confidence: high
---
...
在 NASM 中，常量是通过伪指令定义的符号，汇编器在编译时将所有常量引用替换为实际值。

与变量不同，常量不占用数据段内存，它们在编译时就被替换，相当于 C 语言中的 `#define`。

### 常量 vs 变量

| 特性 | 常量 | 变量 |
| --- | --- | --- |
| 存储位置 | 编译时替换，不占内存 | .data/.bss 段，占用内存 |
| 运行时修改 | ❌ 不可修改 | ✅ 可以修改 |
| 定义方式 | EQU/%define/%assign | DB/DW/DD/RESB/RESW/RESD |
...
使用常量名代替"魔法数字"是一个好习惯，可以大大提高代码的可读性。
...
三个月后，`mov eax, SYS_WRITE` 比 `mov eax, 4` 更容易理解。
