...
tags: [汇编, 变量, x86, 内存]
sources: [raw/articles/汇编语言变量.md]
confidence: high
---
...
- **节省可执行文件空间**：.bss 段的数据不存储在可执行文件中
- **自动初始化为 0**：操作系统在加载程序时会将 .bss 段清零
- **适合大数组和缓冲区**：不会让可执行文件变得臃肿
...
当定义 `value dd 0x12345678` 时，内存中的布局为：
...
| 特性     | 小端序（Little Endian） | 大端序（Big Endian） |
| ------ | ------------------ | ---------------- |
| 存储方式   | 低字节存低地址            | 高字节存低地址         |
| x86 架构 | ✅ 是               | ❌ 否            |
| 人类阅读   | 不太直观              | 更直观            |
...
| 操作符   | 大小   | 示例                   |
| ----- | ---- | -------------------- |
| byte  | 1 字节 | mov byte [var], 1    |
| word  | 2 字节 | mov word [var], 1000 |
| dword | 4 字节 | mov dword [var], 999 |
| qword | 8 字节 | mov qword [var], 123 |
