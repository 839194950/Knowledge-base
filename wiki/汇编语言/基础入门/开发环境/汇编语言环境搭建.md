...
created: 2026-05-17
updated: 2026-05-17
categories: [汇编语言, 基础入门, 开发环境]
categoryPath: "汇编语言/基础入门/开发环境"
tags: [NASM, 开发环境, GDB]
sources: [raw/articles/汇编语言环境搭建.md]
confidence: high
diagramized: true
...
如果你看到版本号输出（如 `NASM version 2.16.01`），说明 NASM 已成功安装。

## 安装链接器

汇编器生成的是**目标文件（.o 文件）**，不能直接运行。你需要链接器将目标文件转换为可执行文件。我们使用 GCC 自带的链接器：
...
你应该会看到类似 `GNU gdb (Ubuntu 12.1-0ubuntu1) 12.1` 的输出。
...
```mermaid
flowchart TD
    A[hello.asm] -->|NASM汇编| B[hello.o]
    B -->|ld链接| C[hello可执行文件]
    C -->|执行| D[输出: Hello runoob]
...
| 2. 链接 | `ld -m elf_i386 hello.o -o hello` | `-m elf_i386` 指定 32 位链接模式，生成可执行文件 hello |
...
如果你在 64 位系统上编译 32 位汇编，需要安装 32 位兼容库：

```bash
sudo apt install gcc-multilib
```

### ld 链接时报错 "cannot find entry symbol _start"

检查你的代码中是否写对了 `global _start` 以及标签名是否完全一致（注意下划线）。

## 相关概念

- [[汇编语言/入门教程/汇编语言简介]] - 汇编语言基础概念
- [[汇编语言/入门教程/汇编语言教程]] - 汇编语言教程概述
